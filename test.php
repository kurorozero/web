...
class testfunction{
    ...
    function open()
    {
        $this->fp = fopen('test_function.txt','w+');
    }

    function __destruct()
    {
        if($this->fp){
            $this->write("operate close \r\n");

        }
    }
}

//$handler = new testfunction();
//$handler->runtest();
...
$type = mime_content_type('php.gif');
//var_dump($type);
$init_time = '15/Mar/2013:01:56:52';
$dateArr = explode("/",substr($init_time,0,11));
$date = $dateArr[2].'-'.$dateArr[1].'-'.$dateArr[0];
$time = substr($init_time,12);
